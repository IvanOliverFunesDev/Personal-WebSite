  ...
  return (
    <>
      <motion.main
        ...
        className="relative flex min-h-screen items-center justify-center bg-[#1E1E2E] text-white"
      >
        {/* Contenido principal con la imagen a la derecha y el texto a la izquierda */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: isLoaded ? 1 : 0 }}
          transition={{ duration: 1, ease: "easeOut" }}
          className={`w-full max-w-6xl px-10 flex items-center justify-between ${!isLoaded && "hidden"}`}
        >
          {/* Sección de texto con animación desde la izquierda */}
          <motion.div
            initial={{ opacity: 0, x: -200 }}
            animate={{ opacity: isLoaded ? 1 : 0, x: isLoaded ? 0 : -200 }}
            transition={{ duration: 1.5, ease: "easeOut" }}
            className="flex flex-col space-y-4 mr-10 max-w-[450px]"
          >
